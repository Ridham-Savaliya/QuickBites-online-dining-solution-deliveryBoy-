...
function OrderCard({ order, onUpdateStatus }) {
  ...
  const [isDelivering, setIsDelivering] = useState(false); // Add missing state
    ...
    <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4">
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isDelivering ? <span>Completing...</span> : <span>Complete Delivery</span>}
          </button>
        </div>
      </div>
...
export default OrderCard;
  ...
  // Add the missing handleDelivery function
  const handleDelivery = async () => {
    setIsDelivering(true);
    try {
      // First send OTP to customer email
      await handleEmail();
      toast.success("Please ask customer for OTP to complete delivery");
    } catch (err) {
      toast.error("Failed to initiate delivery completion");
      console.error(err);
    } finally {
